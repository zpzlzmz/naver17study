...
<div style="margin:20px;">
	<!-- 자바코드로 날짜를 얻어서 JSTL 의 today 변수에 직접 저장 하려면 ?  -->
	...
	<h5>
		급여:
		<%-- <fmt:formatNumber value="${money}" type="currency"/><!-- currency->화폐, number->세번째 자리수 마다, --> --%>
		<fmt:formatNumber value="${money }" type="currency" currencySymbol="￦" maxFractionDigits="0"/>		
		<!-- type= number -> 그냥 숫자 출력 currency 화폐단위 출력, if 화폐단위가 안나올때 해당 단위를 넣어주기, maxfractionDigits -->>
	</h5>
	<h5>
		평균(소수점 이하 자리수 2자리): 
		<fmt:formatNumber value="${average }" maxFractionDigits="2"/>
	</h5>
	<c:set var="num1" value="45.678911"/>
	<c:set var="num2" value="34.1"/>
	<h5>
	...
	<h5>
	<!-- pattern ="0.##" : 두자리로 맞추는데, 값이 없을 경우 출력 안됨 (맞아 떨어지면 출력 안함) -->
		평균(소수점 이하 자리수 2자리): 
